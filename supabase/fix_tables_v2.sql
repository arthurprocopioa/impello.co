-- SCRIPT CORRIGIDO V2 (Garante user_id em Contacts)
...
-- 3. GARANTIR COLUNAS (O pulo do gato 🐱)
do $$ 
begin
    -- Adicionar user_id se faltar (Isso causou o erro 42703!)
    if not exists (select 1 from information_schema.columns where table_name='contacts' and column_name='user_id') then
        alter table public.contacts add column user_id uuid default auth.uid();
    end if;
